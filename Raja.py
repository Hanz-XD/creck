...
\t##`````##````##`````##````##`````##````##`````##
\t##`````##````##`````##````#########````##`````##
			{m}•{k}•{h}• KENDAK KABA LA {u}{m}•{k}•{h}•{u}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def passwrd():
	...
	print(f'>> Mainkan Mode Pesawat Setiap {m}1k{k} Idz\n')
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{k}忠 {u}Gunawkah teserah kaba \n\t{k}忠{u}amaw adaw  Bug/Error Ngiciak awu\n\t{k}忠{u}Doa ka ajaw aku sehat \n\t{k}忠{u}Dapat bini alap\n\t{k}忠{u}Aminkah mangku kamu dapat amal \n\t{k}忠{u}Jangan pacak Coli tula\n\t{k}忠{u}Kalu Dapat Azab , ingatkah awak tu belum nikah\n\t{k}忠{u}Cuman sekedar informasi ajaw\n\t{k}忠{u}Kalu gi kepikiran ndak nikah\n\t{k}忠{u}Tapi amaw ndik gila\n\t{k}忠{u}Lantak ka la nga kaba\n\t{k}忠{u}asak ka kaba senang\n\t{k}忠{u}KURANG SENANG TEMUI AKU\n\t{k}忠{u}DI CINTO MANDI\n\t{k}忠{u}NDIK KRUAN CINTO MANDI? \n\t{k}忠{u}KINAK I GOOGLE MAP\n\t{k}忠{u}JANGAN BIGAL NIAN\n\t{k}忠{u}LA BESAK KABA TU{x}')
	time.sleep(3)
	login()
